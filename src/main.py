...
def main():
    ...
    # TODO potentially save encodings
    encodings: EncodingData = lm.encode(dataset_1["input"])
    ...
    probe.train(dataset=Dataset.from_dict(
        dict(input=encodings, label=dataset_1["label"])))
    # TODO potentially save trained probe
    logger.info("Trained probe")
    ...
    adjectives: Tuple[List[str], List[str], List[str]
                      ] = IOHandler.load_dummy_adjectives()
